...
export const CHARGE_OUTCOMES: readonly OutcomeEntry[] = [
  ...
  },
  {
    ...
    resource: "charge",
    short:
      "Forces an automatic watchtower hold (reason amount_too_large, no code) — the charge lands on_hold immediately and stays non-terminal until released.",
    expectedTerminal: "on_hold",
    timing: "immediate — held before the bank lifecycle; release resumes it to created",
    danger: "safe",
    source: "api-notes §5, §P14",
  },
  {
  ...
  },
  {
    ...
    resource: "charge",
    short:
      "Forces a closed-account bank decline (R02) — and R02 permanently blocks new paykeys on the account it settles against, just like R05, so live runs use a fresh throwaway account.",
    expectedTerminal: "failed",
    returnCode: "R02",
    timing: "~117 s from create to terminal (like B)",
    danger: "poisons",
  ...
  },
  {
    ...
    resource: "charge",
    short:
      "Documented as a settled-then-returned reversal — live it terminates as failed with R02 and never surfaces paid or reversed (the same deviation as Scenario C). R02 also blocks new paykeys on the account it settles against.",
    expectedTerminal: "failed",
    returnCode: "R02",
    timing: "~332 s; created → scheduled → pending → failed, never paid",
    danger: "poisons",
    source: "api-notes §5, §12.18",
  },
];
