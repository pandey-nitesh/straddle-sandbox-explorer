...
7. **Transport.** SDK `@straddlecom/straddle@0.3.0` pinned exact, constructed with explicit `baseURL https://sandbox.straddle.io`, `maxRetries: 0`, `logLevel: 'off'`, and an instrumented custom `fetch`; our wrapper owns retries/backoff and emits `api.exchange` per HTTP attempt and `retry.scheduled` per backoff. The `Idempotency-Key` header is sent explicitly (the SDK's `idempotencyKey` option is inert in 0.3.0). Timestamps vary in precision across endpoints — shared schemas use lenient datetime validation, not bare `z.string().datetime()`.
