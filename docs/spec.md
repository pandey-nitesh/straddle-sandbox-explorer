...
## 19. Learning Layer (added post-Wave 6, 2026-07-07)

The Explorer doubles as a teaching tool for developers new to Straddle. Decisions:

- **Knowledge module `web/src/knowledge/`** — web-only, plain TS, hand-curated **from api-notes.md only** (per §2's M0 rule); every entry carries a `source: "api-notes §n"` citation, and coverage tests assert the on-screen vocabulary (statuses, scenario outcomes, observed return codes) is represented. Deliberately not in `shared/` (prose is not a contract; the §14 freeze must not apply) and never in `server/` (the §8 bundle guard keeps the boundary — `SCENARIO_COPY` mirroring precedent).
- **In-context annotations** attach via the pure projections (`web/src/state/projections.ts`) behind a `{ explain }` option: terminal timeline nodes and return-code chips, wire-log endpoint purposes, and `sandbox_outcome` lines. Components stay note-agnostic; with Explain off the projections emit no note fields at all (tested).
- **Explain toggle** — header ghost button, localStorage-persisted, default on. Sparse-annotation policy and visuals: design.md §6.6.
- **A2 (shipped 2026-07-07):** a "Fields to notice" block below each expanded wire exchange (curated field notes gated on actual presence in the redacted bodies; the `Idempotency-Key` header note keys on endpoint since headers never enter captures); always-visible deviation callouts on live C/D terminal nodes and contract C's provisional-paid node, keyed **structurally** off the `run.started` def snapshot's `requiredObservations` (never label matching); identity/paykey quirk notes in the detail panel. All §12 api-notes deviations are represented in `web/src/knowledge/deviations.ts` (coverage test parses api-notes §12's numbering) plus the spec §18.8/§18.9 gate findings. Replay projections take the same `{ explain }` option so replay annotates identically to live.
- Shipped alongside A2 (same audit): the header key-status pill is live (a slow `/api/health` poll — each hit costs a sandbox request — that also feeds `poller.observeEpoch`, closing §3's "every epoch-carrying response is checked"); consecutive poll failures surface a "server unreachable" chip; startup states re-check health every 2 s so missing/invalid recover without a reload; `GET /api/recordings/:run_id` client parsing now truncates to the valid prefix per §11 instead of aborting; the recordings list refreshes as runs complete; scenario rows are `role="list"`/`listitem` (see design §6.6).
- Remaining increments on the same module, no contract changes: an in-app reference overlay (B); a narrated replay walkthrough over synthetic `__fixtures__` (C). A playground (D) would add a `"playground"` literal to `ScenarioIdSchema`, excluded from reports, gated mock-first, as one §14-synchronized change.
